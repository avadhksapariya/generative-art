  ...
  Widget build(BuildContext context) {
    return ColoredBox(
      ...
      child: SizedBox.expand(
        child: CustomPaint(
          painter: CSRecursiveSquaresGrid(
            sideLength: side,
...
class CSRecursiveSquaresGrid extends CustomPainter {
  CSRecursiveSquaresGrid({
    super.repaint,
    this.sideLength = 80,
    this.strokeWidth = 2,
    this.gap = 10,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int index = 0; index < totalCount; index++) {
      ...
      drawNestedSquare(canvas, Offset((i * (sideLength + gap)), (j * (sideLength + gap))), sideLength, paint, depth);
    }
    canvas.restore();
  }

  void drawNestedSquare(Canvas canvas, Offset start, double sideLength, Paint paint, int depth) {
    ...
    if (enableColors) {
      paint.color = HSLColor.fromAHSL(1, random.nextInt(360).toDouble(), saturation, lightness).toColor();
    }

    canvas.drawRect(Rect.fromLTWH(start.dx, start.dy, sideLength, sideLength), paint);
    ...
    drawNestedSquare(canvas, nextStart, nextSideLength, paint, depth - 1);
  }
